...
if !exists('g:javascript_simplefold_max_depth')
  let g:javascript_simplefold_max_depth = 4
end
let g:javascript_simplefold_prefix = '\v^((^\s{0,' . g:javascript_simplefold_max_depth . '}\S*\s*:.*\S+.*([,\{]\s*$|\_\s*\}))|(^\w.*[\{=])|(^\s*\}))@!.*$'
let g:javascript_simplefold_expr =     '\v(^\s{0,' . g:javascript_simplefold_max_depth . '}\S*\s*:.*\S+.*([,\{]\s*$|\_\s*\}))|(^\w.*[\{=])'
