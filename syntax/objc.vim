...
syn match  objcPragma display "^\s*\%(%:\|#\)\s*pragma\>.*" contains=objcImported 
syn match  objcDefine display "^\s*\%(%:\|#\)\s*define\>.*" 
...
syn keyword objcDirectiveKeyword property synthesize dynamic 
...
\ matchgroup=keyword start="\%(\>\s*\)\@<!\[" start="\%(return\s*\)\@<=\[" end="]" 
\ contains=ALLBUT,@cParenGroup,cErrInParen,cCppParen,cBracket,cCppBracket,@cLabelGroup,cUserCont,@objcAtKeywords 

" A copy of the cNumber and cFloat declarations from c.vim (imported above).
" For some reason, numbers beginning with 0 were being parsed as the beginning
" of a comment block and breaking the syntax highlighting of the rest of the
" file.
syn match objcNumber display contained "\d\+\(u\=l\{0,2}\|ll\=u\)\>"
"hex number
syn match objcNumber display contained "0x\x\+\(u\=l\{0,2}\|ll\=u\)\>"

" Don't flag the first zero of an octal number. If anything's messed up it's
" probably here in these three lines:
syn match	objcNumber		display contained "0\o\+\(u\=l\{0,2}\|ll\=u\)\>" contains=objcNumber
syn match	objcNumber	display contained "\<0"
syn match	objcNumber		display contained "\d\+f"

syn match objcFloat display contained "\d\+f"
"floating point number, with dot, optional exponent
syn match objcFloat display contained "\d\+\.\d*\(e[-+]\=\d\+\)\=[fl]\="
"floating point number, starting with a dot, optional exponent
syn match objcFloat display contained "\.\d\+\(e[-+]\=\d\+\)\=[fl]\=\>"
"floating point number, without dot, with exponent
syn match objcFloat display contained "\d\+e[-+]\=\d\+[fl]\=\>"
if !exists("c_no_c99")
  "hexadecimal floating point number, optional leading digits, with dot, with exponent
  syn match objcFloat display contained "0x\x*\.\x\+p[-+]\=\d\+[fl]\=\>"
  "hexadecimal floating point number, with leading digits, optional dot, with exponent
  syn match objcFloat display contained "0x\x\+\.\=p[-+]\=\d\+[fl]\=\>"
endif
...
if version >= 508 || !exists("did_objc_syntax_inits") 
  ...
  HiLink objcPragma Include 
  HiLink objcDefine Include 
  ...
  HiLink objcNumber Number
  HiLink objcFloat Float
