...
function! s:BufFinderCommands()
  ...
  call s:addfilecmds("initializer", 'ESTD ') " Don't generate RVinitializer to save trouble with Rview
endfunction
...
function! s:readable_related(...) dict abort
  ...
  elseif self.type_name('javascript')
    let sprockets_js = "app/javascripts/application.js"
    let rails_js = "public/javascripts/application.js"
    if filereadable(sprockets_js)
      return sprockets_js
    else
      return rails_js
    endif
